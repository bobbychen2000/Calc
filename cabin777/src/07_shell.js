...
const MAT = {
  ...
  door: { c: '#e4dccb', r: 0.45, l: LAYER.plastic },           // warm cream lining (sans_09 s ~0.12; QA w3) [V: tone]
  ...
  // its bottom grip: a cream band about the bezel colour (sp_py_24 #c7c0ab, underside #9c957a; QA w1: read as a thin
  // grey line) [V]
  shadeRail: { c: '#d6d0c2', r: 0.4 },
...
function recessRay(th, rec = REC) {
  const dx = Math.cos(th), dy = Math.sin(th);
  let lo = 0, hi = 1.0;
  for (let i = 0; i < 28; i++) { const m = (lo + hi) / 2; if (sdRecess(dx * m, dy * m, rec) < 0) lo = m; else hi = m; }
  return [dx * lo, dy * lo];
}
const recessRayY = (th) => recessRay(th, REC_Y);
// band = [bottom rows up to the cell bottom, cell top] (door windows sit higher than the window belt)
function windowCellPattern(cellW, holeW, holeH, holeR, vc, rayFn, band = [WALL.bottomRows, WALL.vb1]) {
  const vb0 = band[0][band[0].length - 1], vb1 = band[1];
  const hw = cellW / 2, hh0 = vc - vb0, hh1 = vb1 - vc;
  ...
  return { ring, sideV, botZ, topZ, hw, vc, rows: band[0], rec: rayFn === recessRayY ? REC_Y : REC };
}
...
// holes: [{ z, pat }] (the J and PY/Y recesses share the cell width, so they mix in one run)
function buildSidewallRun(g, side, za, zb, holes, vTop) {
  const hw = holes.length ? holes[0].pat.hw : 0.2667;
  const topRows = [WALL.vb1, vTop];
  const pats = [...new Set(holes.map((h) => h.pat))];
  const allV = [...WALL.bottomRows.slice(0, -1), ...pats.flatMap((pat) => pat.sideV.map((v) => v + pat.vc)), ...topRows.slice(1)];
  const vset = [...new Set(allV.map((v) => +v.toFixed(5)))].sort((a, b) => a - b);
  const hs = holes.filter((h) => h.z - hw >= za - 1e-6 && h.z + hw <= zb + 1e-6).sort((a, b) => a.z - b.z);
  let cur = za;
  const filler = (z0, z1) => {
  ...
  for (const h of hs) {
    filler(cur, h.z - hw);
    windowCell(g, side, h.z, h.pat, topRows);
    cur = h.z + hw;
  }
  filler(cur, zb);
}
...
function windowCell(g, side, zc, pat, topRows) {
  wallGrid(g, side, pat.botZ.map((z) => zc + z), pat.rows);
...
function ringLoft(rings, map, hint) {
  ...
  for (const [pts, d, mp] of rings) for (const [a, b] of pts) { g.p.push(...(mp || map)(a, b, d)); g.n.push(0, 0, 0); g.u.push(0, 0); }
...
function windowParts(side, pat) {
  ...
  const rec = pat.rec, D = REC.depth;
  // recess rings sink by ext toward the top (0 at the opening's upper corners, full at the recess top)
  const ext = (dv) => rec.ext * clamp((dv - 0.12) / (rec.top - 0.12), 0, 1);
  const mapE = (dz, dv, d) => map(dz, dv, d * (1 + ext(dv) / D));
  const ths = pat.ring.map((p) => p.th);
  const rr = pat.ring.map((p) => p.h);                          // identical to the wall hole ring -> watertight
  ...
  const parts = [
    ...
    [ringLoft([[scl(pane, 1.02), 0.1045], [pane, 0.106]], map, hint), WMAT.seal],
  ];
...
function shadePanelGeo(kind) {
  ...
  if (kind === 'manual') {
    ...
    // pleat depth fades to a flat hem over the outer 35 mm and toward the opening's round corners, so the zig-zag never
    // saw-tooths through the lining (QA w2 / w3: s05 edges and upper corners) [A]
    const g = raw(), n = 62, xs = [-w / 2, -0.125, 0.125, w / 2];
    const W = CAB.win, amp = (x, y) => clamp(-sdRRect(x, y, W.holeW / 2 - 0.004, W.holeH / 2, HOLE_R) / 0.03, 0, 1);
    for (let k = 0; k <= n; k++) {
      const y = -h / 2 + (h * k) / n, zz = (k % 2) * 0.006;       // 6 mm zig-zag (QA w3: carries more of the stripe) [A]
      for (let c = 0; c < 4; c++) { g.p.push(xs[c], y, 0.003 + (zz - 0.003) * (c % 3 ? amp(xs[c], y) : 0)); g.n.push(0, 0, 1); g.u.push(c / 3, k / n); }
    }
...
function windowStudioGeo() {
  ...
  const patY = windowCellPattern(P, W.holeW, W.holeH, HOLE_R, W.yc, recessRayY);
  const zs = [-2.5, -1.5, -0.5, 0.5, 1.5, 2.5].map((k) => k * P);
  const g = raw(); g.dado = raw();
  buildSidewallRun(g, -1, -P * 3, P * 3, zs.map((z, k) => ({ z, pat: k < 3 ? patY : pat })), WALL.vTop);
  ...
  const wp = windowParts(-1, pat), wpY = windowParts(-1, patY);
...
function buildShell(gl, layout) {
  ...
  const patY = windowCellPattern(W.pitch, W.holeW, W.holeH, HOLE_R, W.yc, recessRayY);
  const patOf = (w) => (w.electric ? pat : patY);
  ...
  for (const side of [-1, 1]) {
    ...
    const holes = layout.windows.filter((w) => w.side === side).map((w) => ({ z: w.z, pat: patOf(w) }));
    for (const [za, zb] of runs) {
      const g = raw(); g.dado = raw(); g.dado.jz = jz; g.dado.jb = jr.flat();
      buildSidewallRun(g, side, za, zb, holes, WALL.vTop);
      fixWinding(g); fixWinding(g.dado);
  ...
  // J dado ends that meet a door surround (a run end): close the step between DADO and DADO_J with a cap
  for (const side of [-1, 1]) for (const [a, b] of jr) for (const [z, dir] of [[a, -1], [b, 1]]) {
    if (!runs.some(([ra, rb]) => Math.abs(ra - z) < 1e-3 || Math.abs(rb - z) < 1e-3)) continue;
    const g = raw(), vs = WALL.bottomRows.filter((v) => v <= WALL.dadoTop + 1e-6);
    for (const v of vs) for (const pr of [DADO, DADO_J]) { const [x, y] = wallAt(v, pr); g.p.push(x * side, y, z); g.n.push(0, 0, dir); g.u.push(z, v); }
    for (let j = 0; j < vs.length - 1; j++) { const q = j * 2; g.i.push(q, q + 1, q + 3, q, q + 3, q + 2); }
  ...
  // PY/Y reveals (their taller tub) are baked the same way: the instanced reveal meshes carry the F/J recess
  for (const side of [-1, 1]) {
