...
// ------------------------------------------------------------------
const LAYER = { none: 0, fabric: 1, leather: 2, carpet: 3, plastic: 4, brushed: 5, marble: 6, vinyl: 7, grille: 8, knit: 9, wood: 10, perf: 11, atlasLit: 14, atlasGlow: 15 };
...
const LAYER_PARAMS = {
  ...
  9: [20, 0.4, 0.2, 0.2], 10: [1.5, 0.2, 0.6, 0.3], 11: [7, 0.45, 0.28, 0.3],
};
...
function buildDetailLayers(S = 256) {
  ...
  for (let i = 0; i < 16; i++) L.push(flat);
  ...
  L[10] = makeLayer(S, (u, v) => 0.5, (u, v) => {
    const g = Math.sin((v * 22 + fbm(u * 2, v * 3, 2, 4, 81) * 5) * Math.PI * 2) * 0.5 + 0.5;
