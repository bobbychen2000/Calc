...
// ------------------------------------------------------------------
const SEATMAT = {
  // Economy (Recaro): patterned fabric, colour not documented -> muted blue-grey [A]
  yFabric: { c: '#46546d', r: 0.9, l: LAYER.knit },
  yHead: { c: '#46546d', r: 0.88, l: LAYER.knit },
  yShell: { c: '#3d4147', r: 0.46, l: LAYER.plastic },
  yShellDark: { c: '#2b2e33', r: 0.5, l: LAYER.plastic },
  yTray: { c: '#45494f', r: 0.45, l: LAYER.plastic },
  arm: { c: '#2f3237', r: 0.5, l: LAYER.plastic },
  armPad: { c: '#3a3d43', r: 0.6, l: LAYER.leather },
  ...
  // Premium Economy (ZIM): calm grey-blue fabric with a per-seat pattern [V/A], dark shells
  pyFabric: { c: '#4f5d74', r: 0.88, l: LAYER.knit },
  pyShell: { c: '#34373c', r: 0.44, l: LAYER.plastic },
  pyArm: { c: '#2e3136', r: 0.45, l: LAYER.plastic },
  pyArmPad: { c: '#44474d', r: 0.55, l: LAYER.leather },
  pyTrim: { c: '#9aa0a7', r: 0.3, m: 0.75, l: LAYER.brushed },
  // THE Room (Safran Fusio custom): light Japanese ash exterior, muted dark-grey shells, charcoal textiles,
  // light-grey Ultraleather, rosewood table, deep-blue compartment accents, slate stone
  ash: { c: '#cdbfa6', r: 0.5, l: LAYER.wood },
  ashDark: { c: '#8a7560', r: 0.5, l: LAYER.wood },
  rosewood: { c: '#5c3b2c', r: 0.38, l: LAYER.wood },
  jShell: { c: '#55575b', r: 0.46, l: LAYER.plastic },
  jShellIn: { c: '#6d6f73', r: 0.5, l: LAYER.plastic },
  jFabric: { c: '#4b4e54', r: 0.9, l: LAYER.fabric },
  jLeather: { c: '#a8a8a4', r: 0.52, l: LAYER.leather },
  jHead: { c: '#8e8f8d', r: 0.55, l: LAYER.leather },
  ...
  pillowBlue: { c: '#3e5f97', r: 0.9, l: LAYER.fabric },
  lampGlow: { c: '#ffe2b0', r: 0.4, e: 0.35 },
  moodGlow: { c: '#ffd9a0', r: 0.5, e: 0.25 },
  hole: { c: '#0b0d10', r: 1.0 },
  mirror: { c: '#aeb6bf', r: 0.08, m: 0.9 },
  // THE Suite (JAMCO): grey-brown / bronze shell, dark wood panels, grey door, grey seat with blue accent
  fShell: { c: '#6f6356', r: 0.42, m: 0.15, l: LAYER.brushed },
  fWood: { c: '#4a3629', r: 0.4, l: LAYER.wood },
  fDoor: { c: '#86888a', r: 0.45, l: LAYER.plastic },
  fInner: { c: '#2f3033', r: 0.5, l: LAYER.plastic },
  fFabric: { c: '#707378', r: 0.9, l: LAYER.fabric },
  fLeather: { c: '#5e6166', r: 0.5, l: LAYER.leather },
  fCushionBlue: { c: '#3b5c95', r: 0.9, l: LAYER.fabric },
  fShelf: { c: '#3a2c22', r: 0.35, l: LAYER.wood },
  fWarm: { c: '#ffd6a0', r: 0.5, e: 0.3 },
};
...
function econSeat(B, x0, lod, opts = {}) {
  ...
  if (!opts.noScreen) {
    // 13.3 in touchscreen (0.294 x 0.166) in a bezel, USB + jack below
    B.add(gRBox(0.322, 0.19, 0.014, 0.009, 1), on(0.575, 0.003), SEATMAT.bezel);
    B.add(gQuad(0.294, 0.166), on(0.575, 0.0105), SEATMAT.screen, atlasUV('screen'));
  ...
  for (const s of [-1, 1]) {
    ...
    B.add(gBox(0.045, 0.004, 0.16), M4.trs(x0 + 0.12 * s, 0.471, -0.18, s * 14 * DEG), SEATMAT.strap);
  }
...
function pySeat(B, x0, lod, opts = {}) {
  ...
  for (const s of [-1, 1]) B.add(gRBox(0.05, 0.012, 0.035, 0.005, 1), M4.trs(x0 + 0.03 * s, 0.488, -0.26), SEATMAT.buckle);
}
...
// seat itself (passenger faces -z, back at z = 0): upright or bed
function roomSeatCore(B, bed, lod) {
  const F = SEATMAT.jFabric;
  B.add(gRBox(0.60, 0.33, 0.56, 0.02, 1), M4.trs(0, 0.165, -0.33), SEATMAT.jShell);
  if (bed) {
    B.add(gLoft(cushionSecs(0.62, 0.64, 0.10, -0.05, { edge: 0.035, r: 0.035, crown: 0.004 }), 4), M4.trs(0, 0.38, -0.34), F);
    return;
  }
  B.add(gLoft(cushionSecs(0.62, 0.58, 0.10, -0.05, { edge: 0.04, r: 0.04, crown: 0.012 }), lod ? 3 : 4), M4.trs(0, 0.38, -0.34, 0, 2 * DEG), F);
  const BH = M4.trs(0, 0.43, -0.07, 0, 17 * DEG);
  loftAt(B, [SEC(0.0, 0.58, 0.10, 0, 0.03), SEC(0.06, 0.60, 0.12, -0.01, 0.045), SEC(0.22, 0.60, 0.13, -0.02, 0.05), SEC(0.44, 0.60, 0.12, -0.012, 0.048), SEC(0.58, 0.58, 0.10, 0, 0.04), SEC(0.62, 0.54, 0.07, 0.004, 0.03)], BH, F, lod ? 3 : 4);
  // Ultraleather headrest, reading lights at both top edges, wake-up light under the headrest
  loftAt(B, [SEC(0.60, 0.46, 0.08, 0.004, 0.032), SEC(0.63, 0.50, 0.10, -0.004, 0.042), SEC(0.76, 0.50, 0.10, -0.004, 0.042), SEC(0.80, 0.46, 0.075, 0.002, 0.03), SEC(0.815, 0.42, 0.05, 0.006, 0.02)], BH, SEATMAT.jHead, lod ? 3 : 4);
  if (lod) return;
  for (const s of [-1, 1]) B.add(gCyl(0.018, 0.018, 0.02, 12), M4.mul(BH, M4.trs(s * 0.27, 0.80, 0.02, 0, -Math.PI / 2)), SEATMAT.lampGlow);
  B.add(gBox(0.30, 0.008, 0.02), M4.mul(BH, M4.trs(0, 0.59, 0.04)), SEATMAT.moodGlow);
  // leg rest hanging from the pan front
  B.add(gLoft([SEC(0.0, 0.56, 0.05, 0, 0.02), SEC(0.30, 0.58, 0.055, 0, 0.022)], 3), M4.trs(0, 0.13, -0.64, 0, -18 * DEG), F);
  // shoulder belt
  B.add(gBox(0.045, 0.5, 0.004), M4.mul(BH, M4.trs(-0.2, 0.34, -0.07, 0, 0, 0.35)), SEATMAT.strap);
}
...
function roomPart(part, opts = {}) {
  ...
  if (part === 'O') {
    ...
    // back shell (dark grey) with a light-ash cap toward the aisle
    B.add(gRBox(0.70, wall, 0.07, 0.03, 2), M4.trs(-0.235, wall / 2, -1.31), shell);
    B.add(gRBox(0.64, 0.86, 0.012, 0.01, 1), M4.trs(-0.24, 0.62, -1.272), SEATMAT.jLeather);
    // aisle-side armrest block at the back, with the pop-up privacy panel (retracted)
    B.add(gRBox(0.12, top, 0.26, 0.02, 1), M4.trs(0.135, top / 2, -1.14), SEATMAT.jShellIn);
    B.add(gRBox(0.12, 0.03, 0.26, 0.012, 1), M4.trs(0.135, top + 0.012, -1.14), SEATMAT.jLeather);
    B.add(gRBox(0.36, wall, 0.25, 0.02, 1), M4.trs(0.405, wall / 2, -1.215), ash);
    // side table block over E's footwell (aisle column), door pockets in its aisle wall
    B.add(gRBox(0.45, 0.035, 0.56, 0.01, 1), M4.trs(0.345, top - 0.018, -0.30), ash);
    B.add(gRBox(0.43, 0.02, 0.54, 0.008, 1), M4.trs(0.345, top + 0.002, -0.30), SEATMAT.ashDark);
    ...
    B.add(gRBox(0.28, 0.30, 0.16, 0.015, 1), M4.trs(0.28, top + 0.15, -0.12), ash);
    B.add(gRBox(0.24, 0.24, 0.01, 0.008, 1), M4.trs(0.28, top + 0.15, -0.203), SEATMAT.jShellIn);
    B.add(gRBox(0.10, 0.12, 0.006, 0.004, 1), M4.trs(0.22, top + 0.16, -0.21), SEATMAT.mirror);
    B.add(gRBox(0.04, 0.02, 0.012, 0.004, 1), M4.trs(0.38, top + 0.2, -0.21), SEATMAT.blueAccent);
    // seat controls (knob, slider, presets) on the side table
    if (!lod) {
      B.add(gRBox(0.10, 0.012, 0.18, 0.004, 1), M4.trs(0.22, top + 0.01, -0.46), SEATMAT.black);
      B.add(gCyl(0.022, 0.022, 0.02, 16), M4.trs(0.22, top + 0.02, -0.41), SEATMAT.frame);
      for (let b = 0; b < 3; b++) B.add(gBox(0.016, 0.004, 0.016), M4.trs(0.20 + b * 0.02, top + 0.018, -0.50), { c: '#6d747d', r: 0.4 });
      B.add(gBox(0.02, 0.012, 0.006), M4.trs(0.52, 0.6, -0.40), SEATMAT.port);
    }
    // slate accent strip on the table edge
    B.add(gBox(0.42, 0.012, 0.03), M4.trs(0.345, top + 0.004, -0.575), SEATMAT.slate);
    // sliding door (retracted into the block) and its leading edge
    B.add(gRBox(0.02, wall - 0.14, 0.03, 0.008, 1), M4.trs(0.572, 0.14 + (wall - 0.14) / 2, -0.585), SEATMAT.ashDark);
    ...
    // rosewood table stowed under the monitor + mood light, ottoman in the footwell
    ...
    if (bed) {
      B.add(gRBox(0.64, 0.05, 0.60, 0.02, 1), M4.trs(-0.24, ROOM.bed - 0.025, -0.33), SEATMAT.jFabric);
      ...
      B.add(gLoft(cushionSecs(0.42, 0.28, 0.1, -0.05, { edge: 0.04, r: 0.045 }), 3), M4.mul(M4.trs(-0.24, 0.58, -1.16, Math.PI), M4.trs(0, 0, 0, 0, -70 * DEG)), SEATMAT.pillowBlue);
    }
    // outer skirt (forward half)
    B.add(gRBox(0.025, 0.66, 1.39, 0.01, 1), M4.trs(-0.572, 0.33, -0.66), shell);
  } else {
    ...
    B.add(gRBox(0.62, 0.86, 0.012, 0.01, 1), M4.trs(0.21, 0.62, 1.272), SEATMAT.jLeather);
    // narrow aisle armrest + pop-up privacy panel
    B.add(gRBox(0.09, top, 0.60, 0.02, 1), M4.trs(0.54, top / 2, 0.95), SEATMAT.jShellIn);
    B.add(gRBox(0.09, 0.03, 0.60, 0.012, 1), M4.trs(0.54, top + 0.012, 0.95), SEATMAT.jLeather);
    B.add(gRBox(0.03, 0.06, 0.58, 0.01, 1), M4.trs(0.57, top + 0.05, 0.95), SEATMAT.ash);
    ...
    B.add(gRBox(0.43, 0.02, 0.70, 0.008, 1), M4.trs(-0.355, top + 0.002, 0.93), SEATMAT.ashDark);
    ...
    B.add(gRBox(0.34, 0.30, 0.18, 0.015, 1), M4.trs(-0.33, top + 0.15, 0.66), ash);
    B.add(gRBox(0.30, 0.24, 0.01, 0.008, 1), M4.trs(-0.33, top + 0.15, 0.752), SEATMAT.jShellIn);
    B.add(gRBox(0.10, 0.12, 0.006, 0.004, 1), M4.trs(-0.26, top + 0.16, 0.758), SEATMAT.mirror);
    if (!lod) {
      B.add(gRBox(0.10, 0.012, 0.18, 0.004, 1), M4.trs(-0.30, top + 0.01, 1.02), SEATMAT.black);
      B.add(gCyl(0.022, 0.022, 0.02, 16), M4.trs(-0.30, top + 0.02, 1.07), SEATMAT.frame);
      for (let b = 0; b < 3; b++) B.add(gBox(0.016, 0.004, 0.016), M4.trs(-0.32 + b * 0.02, top + 0.018, 0.98), { c: '#6d747d', r: 0.4 });
    }
    ...
    // E's sliding door leading edge (retracted into the monument)
    B.add(gRBox(0.02, wall - 0.14, 0.03, 0.008, 1), M4.trs(0.572, 0.14 + (wall - 0.14) / 2, 0.035), SEATMAT.ashDark);
    if (bed) {
      B.add(gRBox(0.58, 0.05, 0.62, 0.02, 1), M4.trs(0.20, ROOM.bed - 0.025, 0.36), SEATMAT.jFabric);
      ...
      B.add(gLoft(cushionSecs(0.42, 0.28, 0.1, -0.05, { edge: 0.04, r: 0.045 }), 3), M4.mul(M4.trs(0.20, 0.58, 1.16), M4.trs(0, 0, 0, 0, -70 * DEG)), SEATMAT.pillow);
    }
    B.add(gRBox(0.025, 0.66, 1.33, 0.01, 1), M4.trs(-0.572, 0.33, 0.69), shell);
  }
  return B.build();
}
function roomPartFar(part) {
  ...
  if (part === 'O') {
    ...
    B.add(gBox(0.36, wall, 0.25), M4.trs(0.405, wall / 2, -1.215), SEATMAT.ash);
    B.add(gBox(0.45, top, 0.56), M4.trs(0.345, top / 2, -0.30), SEATMAT.ash);
...
function roomDivider() {
  const B = new Builder();
  B.add(gRBox(0.03, 0.40, 1.30, 0.012, 1), M4.trs(0, 0.86, -0.66), { c: '#c9c3b6', r: 0.4, l: LAYER.plastic });
  B.add(gRBox(0.04, 0.72, 1.30, 0.012, 1), M4.trs(0, 0.36, -0.66), SEATMAT.jShell);
  B.add(gRBox(0.03, 0.36, 1.26, 0.012, 1), M4.trs(0, 0.84, 0.68), { c: '#c9c3b6', r: 0.4, l: LAYER.plastic });
  B.add(gBox(0.034, 0.012, 2.6), M4.trs(0, 1.065, 0.0), SEATMAT.ashDark);
  return B.build();
}
...
function suiteUnit(opts = {}) {
  ...
  B.add(gRBox(w - 0.08, H - 0.12, 0.012, 0.008, 1), M4.trs(0, H / 2 + 0.02, -0.104), SEATMAT.fWood);
  B.add(gRBox(w, H, 0.12, 0.02, 1), M4.trs(0, H / 2, -L + 0.06), SEATMAT.fShell);
  B.add(gRBox(w - 0.1, H - 0.1, 0.012, 0.008, 1), M4.trs(0, H / 2, -L + 0.126), SEATMAT.fWood);
  const scx = (xo + xa) / 2;
  B.add(gRBox(0.99, 0.57, 0.02, 0.01, 1), M4.trs(scx, 0.93, -L + 0.14), SEATMAT.bezel);
  B.add(gQuad(0.952, 0.535), M4.trs(scx, 0.93, -L + 0.151), SEATMAT.screen, atlasUV('screen'));
  B.add(gBox(0.8, 0.008, 0.012), M4.trs(scx, 1.24, -L + 0.14), SEATMAT.fWarm);    // subtle wall light
  // table stowed under the screen on side rails
  B.add(gRBox(0.72, 0.045, 0.06, 0.012, 1), M4.trs(scx, 0.60, -L + 0.17), SEATMAT.fWood);
  // ottoman (full suite width) with belt
  const ow = xa - xo - 0.02;
  B.add(gRBox(ow, 0.38, 0.50, 0.03, 1), M4.trs(scx, 0.19, -L + 0.40), SEATMAT.fInner);
  B.add(gLoft(cushionSecs(ow - 0.02, 0.48, 0.07, -0.035, { edge: 0.03, r: 0.035 }), 3), M4.trs(scx, 0.40, -L + 0.40), SEATMAT.fFabric);
  if (!lod) for (const s of [-1, 1]) B.add(gBox(0.04, 0.004, 0.2), M4.trs(scx + 0.12 * s, 0.441, -L + 0.42), SEATMAT.strap);
  // outer side: low skirt (window) or full-height partition half (centre), plus the long shelf
  ...
  B.add(gRBox(shelfW + 0.01, 0.025, L - 0.30, 0.008, 1), M4.trs(xo + shelfW / 2, 0.652, -L / 2 - 0.04), SEATMAT.fShelf);
  if (!lod) {
    for (const zz of [-0.55, -0.85]) B.add(gRBox(shelfW - 0.04, 0.12, 0.2, 0.008, 1), M4.trs(xo + shelfW / 2, 0.52, zz), SEATMAT.fWood);
    B.add(gRBox(0.10, 0.012, 0.16, 0.004, 1), M4.trs(xo + shelfW / 2, 0.668, -0.62), SEATMAT.black);   // remote dock
    B.add(gRBox(0.02, 0.06, 0.12, 0.004, 1), M4.trs(xo + shelfW + 0.001, 0.58, -1.05), SEATMAT.black); // outlets pocket
  }
  // padded inner armrest on the shelf side
  B.add(gRBox(0.08, 0.10, 0.70, 0.03, 1), M4.trs(xo + shelfW + 0.04, 0.63, -0.50), SEATMAT.fLeather);
  // aisle side: thick wardrobe wall beside the seat + two sliding door panels (open) + pocket at the front
  B.add(gRBox(0.16, H, 0.86, 0.02, 1), M4.trs(hx - 0.08, H / 2, -0.53), SEATMAT.fShell);
  B.add(gRBox(0.012, H - 0.16, 0.80, 0.006, 1), M4.trs(xa - 0.001, H / 2, -0.53), SEATMAT.fWood);
  B.add(gRBox(0.006, 0.9, 0.006, 0.003, 1), M4.trs(xa - 0.008, 0.78, -0.18), SEATMAT.frame);          // wardrobe door line
  B.add(gRBox(0.012, 0.16, 0.03, 0.006, 1), M4.trs(xa - 0.012, 0.95, -0.24), SEATMAT.frame);
  B.add(gRBox(0.012, 0.14, 0.20, 0.006, 1), M4.trs(xa - 0.004, 1.02, -0.72), SEATMAT.fDoor);         // literature pocket / mirror flap
  B.add(gRBox(0.13, H, 0.26, 0.02, 1), M4.trs(hx - 0.065, H / 2, -L + 0.25), SEATMAT.fShell);
  for (const [zc, len] of [[-0.53, 0.5], [-L + 0.26, 0.24]]) B.add(gRBox(0.03, H - 0.12, len, 0.01, 1), M4.trs(hx - 0.012, (H - 0.12) / 2 + 0.06, zc), SEATMAT.fDoor);
  ...
  B.add(gRBox(0.68, 0.34, 0.66, 0.02, 1), M4.mul(S, M4.trs(0, 0.17, -0.36)), SEATMAT.fInner);
  if (bed) {
    ...
    B.add(gRBox(ow, 0.44, 0.9, 0.03, 1), M4.trs(scx, 0.22, -1.40), SEATMAT.fInner);
    ...
    B.add(gLoft(cushionSecs(0.64, 0.62, 0.11, -0.055, { edge: 0.045, r: 0.04, crown: 0.012 }), lod ? 3 : 4), M4.mul(S, M4.trs(0, 0.39, -0.37, 0, 2 * DEG)), F);
    const BH = M4.mul(S, M4.trs(0, 0.44, -0.08, 0, 16 * DEG));
    loftAt(B, [SEC(0.0, 0.62, 0.11, 0, 0.03), SEC(0.06, 0.64, 0.13, -0.01, 0.05), SEC(0.24, 0.64, 0.14, -0.02, 0.055), SEC(0.46, 0.63, 0.13, -0.012, 0.05), SEC(0.62, 0.60, 0.11, 0, 0.042), SEC(0.66, 0.56, 0.08, 0.004, 0.03)], BH, F, lod ? 3 : 4);
    loftAt(B, [SEC(0.64, 0.50, 0.09, 0.004, 0.035), SEC(0.67, 0.54, 0.11, -0.004, 0.045), SEC(0.81, 0.54, 0.11, -0.004, 0.045), SEC(0.85, 0.50, 0.08, 0.002, 0.032), SEC(0.865, 0.46, 0.05, 0.006, 0.02)], BH, SEATMAT.fLeather, lod ? 3 : 4);
    B.add(gLoft(cushionSecs(0.44, 0.1, 0.30, -0.15, { edge: 0.04, r: 0.045 }), 3), M4.mul(BH, M4.trs(0, 0.38, -0.12, 0, 6 * DEG)), SEATMAT.fCushionBlue);
    if (!lod) for (const s of [-1, 1]) B.add(gCyl(0.02, 0.02, 0.02, 12), M4.mul(BH, M4.trs(s * 0.3, 0.86, 0.03, 0, -Math.PI / 2)), SEATMAT.lampGlow);
    B.add(gLoft([SEC(0.0, 0.58, 0.05, 0, 0.02), SEC(0.30, 0.60, 0.055, 0, 0.022)], 3), M4.mul(S, M4.trs(0, 0.12, -0.70, 0, -16 * DEG)), F);
  }
  return B.build();
}
...
function buildSeats(gl, layout) {
  ...
  for (const s of layout.seats) {
    ...
    let p, face;
    if (s.kind === 'room') { p = localToWorld(s, s.odd ? [0.588, 1.0, -1.215] : [0.588, 1.0, 0.95]); face = s.mir ? -1 : 1; if (s.odd) p = localToWorld(s, [0.588, 1.0, -1.215]); }
