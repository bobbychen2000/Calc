...
| 1b | **F9 L1 rotunda** is 1.25 m from its facade point (two-node way): the drum (clamped to 1.5 m) reaches 0.2-0.3 m into the Boarding Area F / ramp-level outline (2-D audit). Same fix as B2. | |
...
| 4 | **A11 upper-deck bridge**: `stow_len` 9.85 m, but the only models that reach U1L (27.5 m) retract to >= 15.15 m (AT3 58/116); resting it at 15.15 m put it in the path of the L2 bridge (777-9 / A350-1000 dockings). gates.js keeps the data's rest pose (AT2 41/55), so it never docks. A rest pose consistent with a model that reaches U1L is needed. | |
