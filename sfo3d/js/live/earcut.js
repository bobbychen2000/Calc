// Polygon triangulation with holes (ear clipping on a linked list, with hole bridging).
// Algorithm after mapbox/earcut (ISC). data: flat [x0,y0,x1,y1,...]; holeIndices: start vertex index of each hole.
  ...
  do {
    ...
    while (b !== a.prev) {
      if (a.i !== b.i && isValidDiagonal(a, b)) {
        ...
        earcutLinked(a, triangles, dim, 0); earcutLinked(c, triangles, dim, 0);
        return;
      }
      b = b.next;
    }
...
function findHoleBridge(hole, outerNode) {
  let p = outerNode; const hx = hole.x, hy = hole.y; let qx = -Infinity, m;
  do {
  ...
  if (!m) return null;
  const stop = m, mx = m.x, my = m.y; let tanMin = Infinity, tan;
  p = m;
  do {
