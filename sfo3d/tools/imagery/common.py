...
World frame (js/geo.js): x = east (m), z = south (m), origin = ARP 37.6188056 N, -122.3754167 E, spherical
equirectangular scale M_PER_DEG_LAT = 110990, M_PER_DEG_LON = 111320 * cos(lat0).  `world_geojs` reproduces that
formula exactly (it is what every data/*.json file and tools/sat measurement was made with).
`world_ltp` is the rigorous alternative: WGS84/GRS80 local tangent plane (ENU) at the same ARP, via pyproj.
...
def world_geojs(lat, lon):
    """js/geo.js llToWorld -> (x east, z south) in metres. Vectorised."""
...
def world_to_utm(x, z, epsg=26910):
    """world (geo.js equirectangular, lat/lon treated as NAD83 - see docs/research/imagery.md) -> UTM E, N."""
    lat, lon = ll_geojs(x, z)
    return ll_to_utm(lat, lon, epsg)


def utm_to_world(E, N, epsg=26910):
    lat, lon = utm_to_ll(E, N, epsg)
    return world_geojs(lat, lon)
...
class NaipSampler:
    """bilinear sampling of the NAIP GeoTIFF (EPSG:26910) at world (x, z) points (geo.js frame, lat/lon as NAD83).
    Uses the exact per-point mapping world -> lat/lon -> UTM -> pixel, so no resampling error is baked in."""
    def __init__(self, year='2024', bands=(1, 2, 3)):
