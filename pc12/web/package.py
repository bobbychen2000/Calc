...
def meshopt(src: Path, dst: Path) -> dict:
    ...
    # ... and their definitions: PBR factors and the KHR_materials_clearcoat / _specular extensions cad/glb.py writes
    # (the viewer's fallback when materials.json is unavailable)
    DEF_PBR = {"baseColorFactor": [1, 1, 1, 1], "metallicFactor": 1, "roughnessFactor": 1}
    DEF_TOP = {"doubleSided": False, "alphaMode": "OPAQUE", "emissiveFactor": [0, 0, 0]}
    DEF_EXT = {"KHR_materials_clearcoat": {"clearcoatFactor": 0, "clearcoatRoughnessFactor": 0},
               "KHR_materials_specular": {"specularFactor": 1, "specularColorFactor": [1, 1, 1]},
               "KHR_materials_ior": {"ior": 1.5}}

    def mat_norm(m):          # glTF defaults filled in (gltf-transform omits them)
        out = {**DEF_TOP, **m, "pbrMetallicRoughness": {**DEF_PBR, **m.get("pbrMetallicRoughness", {})}}
        ext = dict(m.get("extensions", {}))
        for e, d in DEF_EXT.items():
            if e in ext:
                ext[e] = {**d, **ext[e]}
        out["extensions"] = ext
        return norm(out)
    mat_a = {m.get("name"): mat_norm(m) for m in a.get("materials", [])}
    mat_b = {m.get("name"): mat_norm(m) for m in b.get("materials", [])}
    changed = [n for n in used_materials(a) if mat_a.get(n) != mat_b.get(n)]
    if changed:
