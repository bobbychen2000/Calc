...
function updateReadouts() {
  ...
  // doors only move to their lock position (closed up; closed or hanging open down, see pivot.rest) at a
  // lock; a gear stopped (scrubbed) between the locks keeps them open
  const dl = kin.doorLock(g.pos);
  const status = g.pos === 0 && g.door === dl ? 'DOWN' : g.pos === 1 && g.door === dl ? 'UP'
    : g.pos !== g.target ? (g.door < 1 ? 'doors opening' : g.target > g.pos ? 'retracting' : 'extending')
    : dl != null ? (g.door > dl ? 'doors closing' : 'doors opening') : 'stopped · doors open';
