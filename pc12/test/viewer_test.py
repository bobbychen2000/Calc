...
async def numeric_checks(page):
    ...
    r = await js(page, r"""
      ...
      // doors: measured closed (gear locked up), since the GLB may build them open (pivot.rest = 1)
      ...
      out.doorDown = (V.partExtras('gear_door_NR').pivot.rest || 0);
    ...
    check("nose doors open (85 deg, edges below closed) at gear 10..90 %", not bad,
    ...
    for name in ("seqUp", "seqDown"):
        ...
        door_end = 0.0 if target else r["doorDown"]      # doors built open (rest 1) stay open at the down lock
        ...
        ok = not between_closed and first_move is not None and first_move[2] >= 0.999 and end[1] == target and end[2] == door_end
        check(f"gear {'up' if target else 'down'} sequence: doors open -> gear -> doors to lock ({door_end:g})", ok,
...
async def regression_checks(page):
    ...
    r = await js(page, V + r"""
      ...
      return {mid, opening, up, down: document.getElementById('rGear').textContent,
        doorDown: V.partExtras('gear_door_NR').pivot.rest || 0};
    """)
    first = "doors opening" if r["doorDown"] < 1 else "retracting"     # doors built open: straight to retracting
    check("[F5/UX-3] gear readout: scrubbed mid-travel = stopped, doors open", r["mid"].startswith("stopped") and "doors open" in r["mid"]
          and r["opening"].startswith(first) and r["up"].startswith("UP") and r["down"].startswith("DOWN"),
    ...
    n_all = await js(page, "return document.querySelectorAll('#partTree li').length;")
    ...
    check("[UX-2] search: Escape clears the query and the filter; counts show matches / total",
          f1[0] == f"2 of {n_all} parts" and len(f1[1]) == 1 and f1[1][0].startswith("2 / ")
          and f2[0] == f"{n_all} parts" and f2[1] == n_all and f2[2] == "",
